 ...
 * `''`/no matches whenever RAG is off, no workspace is open, `.github/rag`
 * has nothing indexed, or `plan` has no operations — every one of those is
 * a silent, normal no-op, never surfaced as an error.
 */
export async function packRagSection(plan: OperationPlan, context: RagPackingContext): Promise<{ section: string; matches: RagMatch[] }> {
  const { extensionContext, settings, mandatoryTokens, model, cancellationToken, logPrefix, onLog } = context;
  ...
  const staleFilePaths = await getStaleRagFilePaths(workspaceRoot, context);
